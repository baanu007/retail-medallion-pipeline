...
Gold metric: Sales trends — monthly rollups.

Grain: 1 row per (year, month, restaurant_id)
Feeds dashboard KPI cards and trend line charts.
"""
...
GOLD_PATH = f"s3://{S3_BUCKET}/gold/gold_sales_monthly"
TABLE_LABEL = "gold_sales_monthly"
...
try:
    ...
    monthly = (
        fact
        .groupBy("order_year", "order_month", "order_year_month", "restaurant_id")
        .agg(
            ...
            F.round(F.sum("total_option_revenue"), 2).alias("total_option_revenue"),
        )
        .withColumn("gold_ingestion_ts", F.lit(RUN_TS))
    )

    n = monthly.count()
    logger.info(f"  Monthly rows: {n:,}")
